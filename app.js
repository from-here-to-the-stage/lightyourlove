...
function parseShowDates(cityData) {
    const dates =[];
    let baseTimeStr = '';
    let baseYear = parseInt(cityData.fd.split('-')[0], 10);
    
    // st: '2026-05-07T20:00:00-06:00' からタイムゾーンを含む 'T20:00:00-06:00' を抽出
    if (cityData.st) {
        const tIndex = cityData.st.indexOf('T');
        if (tIndex !== -1) {
            baseTimeStr = cityData.st.substring(tIndex);
        }
    }
    
    cityData.shows.forEach((showStr) => {
        const [month, day] = showStr.split('/').map(Number);
        
        let y = baseYear;
        const fdMonth = parseInt(cityData.fd.split('-')[1], 10);
        // 年跨ぎ（北米からアジアへ戻る際など）の考慮
        if (fdMonth >= 10 && month <= 3) {
            y = baseYear + 1;
        }
        ...
        let showDate;
        if (baseTimeStr) {
            // 文字列結合でタイムゾーン込みの現地時間を正確にパース
            showDate = new Date(dateStr + baseTimeStr);
        } else {
            // stが未定の場合はUTC 12:00として扱う
            showDate = new Date(Date.UTC(y, month - 1, day, 12, 0, 0));
        }
        dates.push(showDate);
    });
    return dates;
...
// 動的に現在時刻に基づいて公演のステータスを更新
function updateTourStatus() {
    const now = new Date();
    let foundNext = false;
    
    TOUR.forEach(v => {
        if (!v.showDates) return;
        ...
        v.showDates.forEach((showTime, idx) => {
            const diff = showTime - now;
            // 開演2時間後まではその公演を「現在」として扱う
            if (diff > -7200000) {  
                allPast = false;
                if (diff < minDiff) {
                    minDiff = diff;
...
// 対象の現地公演日付をキーにして1日目・2日目を区別する
const showKey = v => {
    if (v.shows && v.nextShowIndex >= 0 && v.nextShowIndex < v.shows.length) {
        let year = parseInt(v.fd.split('-')[0], 10);
        const [mStr, dStr] = v.shows[v.nextShowIndex].split('/');
        const month = parseInt(mStr, 10);
        const fdMonth = parseInt(v.fd.split('-')[1], 10);
        if (fdMonth >= 10 && month <= 3) year += 1;
        return `${year}-${mStr.padStart(2,'0')}-${dStr.padStart(2,'0')}_${v.city}`;
    }
    return `${v.fd}_${v.city}`;
};
...
const SDS = {
  ...
  async sign(salt, fingerprint, showKeyStr, nonce, date) {
    const enc = new TextEncoder();
    const msg = enc.encode(salt + fingerprint + '|' + showKeyStr + '|' + nonce + '|' + date);
  ...
  isLocalSent(showKeyStr) {
    const today = this.jstDateStr();
    const local   = this.loadLocal();
    const session = this.loadSession();
    const lr = local[showKeyStr];
    const sr = session[showKeyStr];
    if(lr?.date === today) return true;
    if(sr?.date === today) return true;
    return false;
  },

  async sendToGAS(showKeyStr, memberId, fingerprint, sig, nonce, date) {
  ...
  async checkAndSend(showKeyStr, memberId) {
    ...
    if(this.isGASEnabled()) {
      ...
      const sig   = await this.sign(salt, fp, showKeyStr, nonce, date);

      try {
        const result = await this.sendToGAS(showKeyStr, memberId, fp, sig, nonce, date);

        if(result === 'Success') {
          this.saveLocal(showKeyStr, memberId, sig, nonce, date);
      ...
      if(salt) {
        ...
        this.saveLocal(showKeyStr, memberId, 'unsigned', 'no-salt', date);
      }
...
const DailyToken = {
  ...
  showKeyFor(v) { return showKey(v); },
  async isAlreadySent(showKeyStr) { return SDS.isLocalSent(showKeyStr); },
  async recordSend(showKeyStr, fp, memberId) {
    SDS.saveLocal(showKeyStr, memberId, 'post-send', 'bridge', SDS.jstDateStr());
  }
};
...
// 開演後2時間までは送信済みでもターゲットとして維持する
function findNextTarget(){
  updateTourStatus();
  const now = new Date();
  
  // 1. カウントダウン中〜公演終了（開演2時間後）までの公演は、送信済みかどうかにかかわらずターゲットとして維持
  for(const v of TOUR){
    if(!v.st || v.isTBA) continue;
    if(v.nextShowIndex >= 0) {
      const diff = v.showDates[v.nextShowIndex] - now;
      if(diff > -7200000 && diff <= WINDOW_MS) return v;
    }
  }
  
  // 2. 直近の「next」ステータスの未送信公演
  for(const v of TOUR){
...
function selectStop(v,i){
  try {
    ...
    // 表示用の日付は現在の未開催公演日を使用
    let d;
    if (v.showDates && v.nextShowIndex >= 0) {
        d = v.showDates[v.nextShowIndex];
    } else if (v.showDates && v.showDates.length > 0) {
        d = v.showDates[v.showDates.length - 1];
    } else {
        d = new Date(v.fd+'T12:00:00Z');
    }
    ...
    if(tDetail) tDetail.innerHTML =
      `<span class="txt-jp">${MONJP[d.getMonth()]}${d.getDate()}日 · ${v.venue}</span>`+
      `<span class="txt-en" style="${lang==='jp'?'display:none':''}">${_mons[d.getMonth()]} ${d.getDate()} · ${v.venue}</span>`;
...
function drawMap(){
  try {
    ...
    if(!mc._bgCache || mc._bgCache.w!==W || mc._bgCache.h!==H){
      ...
      octx.strokeStyle='rgba(91,63,217,.08)';
      octx.lineWidth=.5;
      ...
      octx.setLineDash([]);
      octx.strokeStyle='rgba(91,63,217,.14)';
      octx.lineWidth=.7;
      octx.beginPath();
      const[,eq]=ll(0,0,W,H);
      octx.moveTo(0,eq); octx.lineTo(W,eq); octx.stroke();
      
      octx.beginPath();
      const[pm]=ll(0,0,W,H);
      octx.moveTo(pm,0); octx.lineTo(pm,H); octx.stroke();
...
function updateCompassFromUser(){
  try {
    ...
    console.warn('[Compass] updateCompassFromUser failed:', e);
  }
}
...
function requestGeo(){
  try {
    ...
    if(btn){
      ...
      btn.innerHTML = lang==='jp' ? '<span>📍 取得中…</span>' : '<span>📍 Locating…</span>';
    }
    ...
    navigator.geolocation.getCurrentPosition(
      pos => {
        ...
        if(btn){
          ...
          btn.innerHTML = lang==='jp' ? '<span>📍 現在地で測定</span>' : '<span>📍 MY LOCATION ✓</span>';
        }
      },
      err => {
        geoGranted = false;
...
function onOrient(e){
  try {
    ...
    console.warn('[Compass] Orientation event failed:', err);
  }
}
...
function rotateNeedle(a){
  try {
    ...
    console.warn('[Compass] Rotate needle failed:', e);
  }
}
  ...
  try {
    ...
    if(countEl){
      if(todaySelected && selectedMember){
        countEl.textContent = lang==='jp' ? `${selectedMember.jp||selectedMember.name} ✓` : `${selectedMember.name} ✓`;
        countEl.style.color = selectedMember.color;
      } else {
        countEl.innerHTML = lang==='jp' ? '<span class="txt-jp">未選択</span>' : '<span class="txt-en">NOT YET</span>';
...
function playSendWhoosh(color){
  try {
    ...
    console.warn('[Audio] Whoosh failed:', e.message);
  }
}
  ...
  try {
    ...
    const sparks = Array.from({length:80},(_,i)=>{
      ...
      return {
        ...
        life:1,decay:.014+Math.random()*.01,size:2+Math.random()*4
      };
    });
...
function tryShowCountdown(){
  try {
    if(!targetStop || !targetStop.showDates || targetStop.nextShowIndex < 0) return;
    const sk = showKey(targetStop);
    const showTime = targetStop.showDates[targetStop.nextShowIndex];
    const diff = showTime - new Date();
    ...
    if(diff > 0 && diff <= 3600000 && !countdownShownKey.has(sk)){
      ...
      openCountdownOverlay(targetStop, showTime);
    }
  } catch (e) {
...
function openCountdownOverlay(stop, showTime, isDemo=false){
  try {
    ...
    if(!showTime) {
      if (stop.showDates && stop.nextShowIndex >= 0) {
        showTime = stop.showDates[stop.nextShowIndex];
      } else return;
    }
...
function drawCountdownBg(canvas, col){
  try {
    ...
    for(let i=0;i<80;i++){
      const x=Math.random()*W, y=Math.random()*H, sz=Math.random()*1.5;
...
function refreshStatusBar(){
  try {
    ...
    if(ctEl && nxt && nxt.showDates && nxt.nextShowIndex >= 0){
      const diff = nxt.showDates[nxt.nextShowIndex] - new Date();
...
function evaluateSendState(){
  try {
    ...
    if(targetStop.isTBA || !targetStop.showDates || targetStop.nextShowIndex < 0){
      if (targetStop.nextShowIndex < 0 && targetStop.status === 'past') {
        applyExpiredState();
      } else {
        applyTBAState();
      }
    ...
    const nextShow = targetStop.showDates[targetStop.nextShowIndex];
    ...
    // 開演1時間前（WINDOW_MS）〜開演2時間後までは送信可能
...
function refreshStep(forceStep){
  try {
    ...
    let diff2 = null;
    if (targetStop && targetStop.showDates && targetStop.nextShowIndex >= 0) {
      diff2 = targetStop.showDates[targetStop.nextShowIndex] - new Date();
    }
    ...
    if(step===3){
      ...
      } else if(targetStop.nextShowIndex < 0){
        statusEl.textContent = lang==='jp'?'この都市の全公演が終了しました':'All shows in this city have ended';
        statusEl.style.color = 'var(--dim)';
      } else if(diff2!=null && diff2 < -7200000){
...
function updateSendBtnText(){
  try {
    ...
    const expired = (diff2 !== null && diff2 < -7200000 && !isSent(targetStop)) || (targetStop.nextShowIndex < 0);
    
    if(cls==='ready'){
      const m=diff2!=null?Math.max(0,Math.floor(diff2/60000)):0;
      const pwr=Math.round(getPower());
      btn.textContent=lang==='jp'
        ?`✦ 光を送る · ${activeMember.jp}カラー · 残${m}分 · PWR${pwr}`
        :`✦ SHOOT LIGHT · ${activeMember.name} · ${m}min · PWR${pwr}`;
    } else if(cls==='sent'){
      const sm=sentMem(targetStop)||activeMember;
      btn.textContent=lang==='jp'
        ?`✦ ${sm.jp||sm.name}カラーで送信済み · 次の公演でも送れます`
        :`✦ Sent as ${sm.name} · Available at next show`;
    } else if(cls==='tba-lock'){
      if (targetStop.nextShowIndex < 0 && targetStop.status === 'past') {
          btn.textContent=lang==='jp'?'— この都市の公演は終了しました':'— Shows in this city have ended';
      } else {
      ...
      const h=diff2!=null?Math.ceil(diff2/3600000):null;
      btn.textContent=lang==='jp'
        ?(h!=null&&h<=24?`🔒 あと${h-1}時間で送信ウィンドウが開きます`:'🔒 開演1時間前に送信可能')
        :(h!=null&&h<=24?`🔒 Window opens in ${h-1}h`:'🔒 UNLOCKS 1H BEFORE SHOWTIME');
    }
  } catch (e) {
...
async function doSend(){
  try {
    ...
    const btn=getEl('sendBtn');
    if(btn) { btn.style.cursor = ''; btn.disabled = false; }
  }
}
...
function initSendBtn(){
  try {
    ...
    console.warn('[SendBtn] Init failed:', e);
  }
}
...
function updateCountdown(){
  try {
    const nxt = (typeof findNextTarget === 'function') ? findNextTarget() : targetStop;
    if(!nxt || !nxt.showDates || nxt.nextShowIndex < 0){
      setText('mainCnt', '—');
      return;
    }
    const diff = nxt.showDates[nxt.nextShowIndex] - new Date();
...
function updateShowtime(){
  try {
    ...
    if (targetStop && targetStop.showDates && targetStop.nextShowIndex >= 0) {
        diff = targetStop.showDates[targetStop.nextShowIndex] - new Date();
        if (showtimeCnt) showtimeCnt.textContent = fmt(diff);
    } else if (targetStop && targetStop.showDates && targetStop.nextShowIndex < 0) {
        if (showtimeCnt) showtimeCnt.textContent = lang === 'jp' ? '終了' : 'ENDED';
    } else if (!targetStop.st) {
        if (showtimeCnt) showtimeCnt.textContent = fmt(null);
    }
    
    const ctEl = getEl('sbCountdown');
    const ncEl = getEl('sbNextCity');
    if (ncEl && _fresh) ncEl.textContent = _fresh.city;
    if (ctEl && _fresh && _fresh.showDates && _fresh.nextShowIndex >= 0) {
         const fDiff = _fresh.showDates[_fresh.nextShowIndex] - new Date();
         ctEl.textContent = fDiff > 0 ? fmt(fDiff) : (lang==='jp'?'開演中':'LIVE NOW');
    }

    if (diff !== null && !isSent(targetStop) && !targetStop.isTBA) {
        // 開演2時間後（-7200000ms）までは送信枠内
        if (diff > -7200000 && diff <= WINDOW_MS && !sendReady) {
            activateSendReady();
        } else if (diff <= -7200000 && sendReady) {
            applyExpiredState();
        }
    }
...
function checkShowtimeArrival(){
  try {
    const now=new Date();
    TOUR.forEach(v=>{
      if(!isSent(v) || !v.showDates || v.nextShowIndex < 0) return;
      const showTime = v.showDates[v.nextShowIndex];
      const k=showKey(v);
...
function buildShareCanvas(){
  try {
    const c=getEl('shareCanvas');
    const ctx=c?c.getContext('2d'):null;
    if(!c || !ctx) return;
    
    const W=600,H=280;
    const bg=ctx.createLinearGradient(0,0,W,H);
    bg.addColorStop(0,'#EEF0FF');
    bg.addColorStop(1,'#E8EAFF');
    ctx.fillStyle=bg;
    ctx.fillRect(0,0,W,H);
    
    for(let i=0;i<95;i++){
      ctx.fillStyle=`rgba(255,255,255,${(Math.random()*.2+.04).toFixed(2)})`;
      ctx.beginPath();
      ctx.arc(Math.random()*W,Math.random()*H,Math.random()*1.3,0,Math.PI*2);
      ctx.fill();
    }
    
    ctx.strokeStyle='rgba(155,114,240,.12)';
    ctx.lineWidth=.8;
    for(let lo=-180;lo<=180;lo+=30){
      ctx.beginPath();
      const x=((lo+180)/360)*(W-36)+18;
      ctx.moveTo(x,0);
      ctx.lineTo(x,H);
      ctx.stroke();
    }
    ...
    segs.forEach(seg=>{
      ...
      seg.forEach(([la,lo])=>{
        const x=((lo+180)/360)*(W-36)+18;
        const y=((90-la)/180)*(H-36)+18;
        if(ff){ctx.moveTo(x,y);ff=false;}else ctx.lineTo(x,y);
      });
      ctx.shadowBlur=12;
      ctx.shadowColor=activeMember.color;
      ctx.strokeStyle=activeMember.color+'cc';
      ctx.lineWidth=lw;
      ctx.setLineDash([6,6]);
      ctx.stroke();
      ctx.setLineDash([]);
    ...
    [[routeFrom,'#EAC130'],[routeTo,activeMember.color]].forEach(([v,col])=>{
      const x=((v.lng+180)/360)*(W-36)+18;
      const y=((90-v.lat)/180)*(H-36)+18;
      ctx.shadowBlur=14;
      ctx.shadowColor=col;
      ctx.fillStyle=col;
      ctx.beginPath();
    ...
    ctx.font='bold 27px Chakra Petch,monospace';
    ctx.fillStyle='#D4AF37';
    ctx.shadowBlur=16;
    ctx.shadowColor='rgba(176,125,16,.5)';
    ctx.fillText('✦ FHTS',20,42);
    ctx.shadowBlur=0;
    
    ctx.font='9px Chakra Petch,monospace';
    ctx.fillStyle='rgba(255,255,255,.3)';
    ctx.fillText('LIGHT ROUTE · GLOBAL EDITION 2026–27',20,58);
    
    const b2=Math.round(brng(routeFrom.lat,routeFrom.lng,routeTo.lat,routeTo.lng));
    const d2=Math.round(hav(routeFrom.lat,routeFrom.lng,routeTo.lat,routeTo.lng));
    ctx.fillStyle='rgba(255,255,255,.2)';
    ctx.font='9px Chakra Petch,monospace';
    ctx.fillText(`${b2}° · ${d2.toLocaleString()} km · #BTSLightRoute`,20,H-14);
    ...
    console.warn('[Share] Canvas build failed:', e);
  }
}
...
function initShare(){
  try {
    ...
    if(closeShare) closeShare.addEventListener('click',()=>{
    ...
    document.querySelectorAll('.spbtn').forEach(btn=>{
      ...
      btn.addEventListener('click',async()=>{
        try{
          ...
          const file=new File([blob],'bts-light-route.png',{type:'image/png'});
          const text=`✦ BTS LIGHT ROUTE · ${routeFrom.city.toUpperCase()} → ${routeTo.city.toUpperCase()} · #BTSLightRoute`;
          ...
          console.warn('[Share] API failed, falling back:', e);
          const sc=getEl('shareCanvas');
          if(!sc) return;
          const a=document.createElement('a');
          a.href=sc.toDataURL('image/png');
    ...
    console.warn('[Share] Init failed:', e);
  }
}
...
async function init(){
  try {
    ...
    if(mc && mc.width === 0 && mc.parentElement){
      mc.width = mc.parentElement.offsetWidth || 358;
      mc.height = 200;
    }
