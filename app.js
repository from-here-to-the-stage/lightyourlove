...
function updateTourStatus() {
    ...
    TOUR.forEach(v => {
        ...
        v.showDates.forEach((showTime, idx) => {
            ...
            if (diff > -7200000) {  // 開演2時間後までは現在/未来として扱う
                allPast = false;
                if (diff < minDiff) {
                    minDiff = diff;
                    nextIdx = idx;
...
// ════════════════════════════════════════════════
// PER-SHOW SENT STATE (Moved up for dependencies)
// ════════════════════════════════════════════════
const SENT_KEY = 'bts_lr_sent_v5';
let sentShows = {};
try { sentShows = JSON.parse(localStorage.getItem(SENT_KEY)||'{}'); } catch(e) {}

// ★ 1日目・2日目で別々に送信を管理するため、対象公演の年月日をキーにする
const showKey = v => {
    if (v.showDates && v.nextShowIndex >= 0) {
        const d = v.showDates[v.nextShowIndex];
        return `${d.getUTCFullYear()}-${String(d.getUTCMonth()+1).padStart(2,'0')}-${String(d.getUTCDate()).padStart(2,'0')}_${v.city}`;
    }
...
function selectStop(v,i){
  try {
    ...
    // 表示用の日付は現在の未開催公演日を使用
    const d = (v.showDates && v.nextShowIndex >= 0) ? v.showDates[v.nextShowIndex] : new Date(v.fd+'T12:00:00Z');
    ...
    MEMBERS.forEach(m => {
      ...
      btn.addEventListener('click', () => {
        ...
        if(result.action === 'locked') {
          btn.animate([{transform:'translateX(-3px)'},{transform:'translateX(3px)'},
...
function playCrowdCheer(duration=4.0, volume=0.18){
  try {
    ...
    noise.connect(bp); bp.connect(lp); lp.connect(gain); gain.connect(out);
...
function drawChargeParticles(canvas, color){
  try {
    ...
    const particles = Array.from({length:40}, (_,i)=>{
      ...
      return { x:cx, y:cy, vx: Math.cos(angle)*speed, vy: Math.sin(angle)*speed, life:1, decay: 0.018+Math.random()*0.012, size: 3+Math.random()*5 };
    });
...
function drawSendParticles(canvas, color){
  try {
    ...
    const streaks = Array.from({length:60},(_,i)=>{
      ...
      return { angle, len:0, maxLen:60+Math.random()*120, speed:3+Math.random()*4, life:1, delay:Math.random()*15 };
    });
...
function openCountdownOverlay(stop, showTime, isDemo=false){
  try {
    ...
    if(!showTime) {
      if (stop.showDates && stop.nextShowIndex >= 0) showTime = stop.showDates[stop.nextShowIndex];
      else return;
    }
    ...
    function updateTimer(){
      try {
        ...
        if(rem <= 0){
          ...
          clearInterval(countdownTimer);
          if(!isDemo) setTimeout(()=> ov.classList.remove('on'), 3000);
          return;
        }
...
function updateShowtime(){
  try {
    ...
    if (diff !== null && !isSent(targetStop) && !targetStop.isTBA) {
        if (diff > 0 && diff <= WINDOW_MS && !sendReady) {
            activateSendReady();
        } else if (diff <= 0 && sendReady) {
            applyExpiredState();
        }
    }
...
async function init(){
  try {
    ...
    window.addEventListener('resize', ()=>{ if(typeof resizeMap === 'function') resizeMap(); });
...
window.addEventListener('DOMContentLoaded', () => { 
  try { init(); GA.init(); } catch (e) { console.error('[Bootstrap] Fatal error:', e); }
});