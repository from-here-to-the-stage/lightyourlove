...
const GA = {
  ...
  init() {
    try {
      // gtag が未定義（GA4 スクリプト未読み込み or ブロック）の場合は早期リターン
      if (typeof gtag === 'undefined') {
        console.warn('[GA] gtag is not defined — GA4 script may be missing or blocked');
        return;
      }
  ...
  grant(save = true) {
    try {
      ...
      // 自動ページビュー送信が GA4 config 側で有効化されているため、
      // 手動の gtag('event','page_view',...) 送信は重複計測になるため削除。
      // 同意更新後は GA4 が次回ナビゲーション/ページロード時に自動送信する。
    } catch (e) {
